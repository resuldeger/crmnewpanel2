 ...
 *
 * Narrowed to the caller's own branches, because assigning across a
 * boundary they cannot see would let them read the name of someone at a
 * studio they have no access to.
 * ────────────────────────────────────────────────────────────────── */
export const GET = withAuth("calls.view", async (user, req: NextRequest) => {
  ...
  const rows = await db
    ...
    .where(and(...[eq(staff.active, true), sharesABranch].filter(Boolean as never as <T>(x: T) => x is T)))
