...
import { FormStep, type PiercingArea } from '../types';
import { t, type StepOption } from '../hooks/useBookingConfig';
...
import { labelInZone, crossesDay, zoneAbbr, dateInZone } from '../lib/slotTime';
...
export interface BookingWizardProps {
  /** studio slug from the URL segment */
  slug: string;
}

const BookingWizard: React.FC<BookingWizardProps> = ({ slug }) => {
  ...
  const {
    ...
    /* Steps do not change the address. They push a history entry at the
       same URL so Back still walks the wizard instead of leaving the site,
       which is what the address-per-step version was really for. */
    onStepChange: (next) => {
      if (typeof window === 'undefined') return;
      try {
        window.history.pushState({ cleoStep: next }, '', window.location.href);
      } catch {
        /* sandboxed iframe — the wizard still works, Back just exits */
      }
    },
  });


  /* Back/Forward walk the steps. Each step pushed an entry at the same
     URL, so popstate means "the visitor pressed Back" — step back rather
     than let the browser leave the booking flow. */
  useEffect(() => {
    const onPop = () => {
      if (step === FormStep.SUCCESS) return; // the booking is already made
      prevStep();
    };
  ...
  useEffect(() => {
    if (config && config.location.bookingActive === false) window.location.replace("/");
  }, [config]);
  ...
  const studioTz = config?.location.timezone ?? 'UTC';
  const studioAbbr = zoneAbbr(studioTz);
        ...
        <div className="animate-reveal py-4">
                ...
                <div className="grid grid-cols-2 md:grid-cols-1 gap-3 max-w-xl mx-auto">
                  {(() => {
                    const piercingOpts = (config?.steps?.style ?? [])
                      .filter((opt: StepOption) => ['ear', 'face', 'mouth', 'body', 'dermal', 'genital'].includes(opt.key.toLowerCase()));
          ...
          {step === FormStep.SIZE && (
            <div className="space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-1 gap-3 max-w-xl mx-auto">
                {(config?.steps?.size ?? []).map((opt: StepOption) => (
                  <SelectionButton 
                    key={opt.key}
                    label={opt.label} 
            ...
            <div className="space-y-4 max-w-lg mx-auto pb-10">
              ...
              <div ref={timeSectionRef} className="animate-reveal space-y-4 pt-6 max-w-[340px] mx-auto scroll-mt-24">
                  ...
                  <div className="space-y-6">
                    ...
                    {/* "Today" has to mean today AT THE STUDIO. Built from the
                        browser clock, a visitor in Istanbul booking Tacoma saw
                        the two-hour notice on the wrong date — missing on the
                        day the rule actually bites, present on a day it does
                        not. The backend has always used studio-local time. */}
                    {formData.selectedDate === dateInZone(new Date().toISOString(), studioTz) && (
                    ...
                    {availLoading ? (
                      ...
                      timeSlots.map(slot => {
                        // The button shows the visitor's own clock; the
                        // studio's time is kept underneath when the two
                        // differ, so nobody books the wrong hour.
                        const visitorTz = formData.timezone || studioTz;
                        const shown = slot.startsAt ? labelInZone(slot.startsAt, visitorTz) : slot.time;
                        const differs = shown !== slot.time;
                        const nextDay = slot.startsAt ? crossesDay(slot.startsAt, studioTz, visitorTz) : false;
                        return (
                        <button 
                          key={slot.startsAt ?? slot.time} 
                        ...
                        >
                          <span className="block">{shown}{nextDay && <sup className="ml-0.5 text-[9px]">+1</sup>}</span>
                          {differs && (
                            <span className={`mt-0.5 block text-[9px] font-bold tracking-normal ${formData.selectedTime === slot.time ? 'text-black/60' : 'text-zinc-600'}`}>
                              {slot.time} {studioAbbr}
                            </span>
                          )}
                          {slot.booked && (
            ...
            <div className="space-y-8">
              ...
              <form className="max-w-md mx-auto space-y-5" onSubmit={(e) => e.preventDefault()}>
                ...
                <PhoneInputField
                  ...
                  translations={config?.translations}
                  defaultCountry={config?.location.countryCode as never}
                />
