...
export interface SlotInfo {
  /** Studio wall-clock time. */
  time: string;
  /** The same moment as a UTC instant — render this in the visitor's zone. */
  startsAt: string;
  booked: boolean;
}
  ...
  useEffect(() => {
    const fetchAvailability = async () => {
      ...
      try {
        ...
        // `throw "boom"` is legal, so err.message can be undefined and the
        // banner renders empty. Fall back to something the visitor can act on.
