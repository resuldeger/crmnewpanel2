...
export function useTracking(locationSlug: string, currentStep: number, formData?: FormData, locale = 'en') {
  const [sessionUuid, setSessionUuid] = useState<string | null>(null);
  /** Set once the funnel has captured enough contact detail to create a lead. */
  ...
  const sendStepUpdate = (stepToSend: number, dataToSend?: FormData) => {
    ...
    .then((data: { lead_id?: string | null } | null) => {
      /* capture_lead() runs the moment a usable phone or email arrives, so
         this is the first point at which the visitor is a real, reachable
         person rather than an anonymous browser. The reference image waits
         for it before being uploaded. */
      if (data?.lead_id) setLeadId(prev => prev ?? data.lead_id!);
    })
  ...
  /* Heartbeat for the live-visitor board. A visitor reading a step sends
   * no step updates, so without this they would look as though they had
   * left. One column, no payload, and it stops when the tab is hidden. */
  useEffect(() => {
    if (!sessionUuid) return;

    const ping = () => {
      if (document.visibilityState !== 'visible') return;
      void fetch(api(`/api/booking/sessions/${sessionUuid}/ping`), {
        method: 'POST',
        keepalive: true,
      }).catch(() => undefined);
    };
