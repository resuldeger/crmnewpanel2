...
export default function SmsCompose({ leadId, phone, name, locationId, dateOverride, onSent, onClose }: {
  ...
  const dateStr = dateOverride ?? (appt ? `${fmtD(appt.preferredDate)} at ${appt.preferredTime}` : t("a slot this week"));

  const filled = (body: string) =>
    body
      .replace(/\{customer_name\}/g, cName.split(" ")[0])
      .replace(/\{location_name\}/g, studio?.name ?? t("our studio"))
  ...
  const send = () => {
    ...
    toast(tf("SMS queued to {name} via Twilio", { name: cName.split(" ")[0] }));
    onSent?.();
    onClose();
  ...
  return (
    <Modal onClose={onClose} w={640}>
      <ModalHead
        title={t("Send SMS")}
        sub={<span className="num">{tf("To {name} · {phone} · from {sender}", { name: cName, phone: prettyPhone(cPhone) || t("no phone"), sender: studio?.twilioNumber ?? "Twilio" })}</span>}
        onClose={onClose}
      />
      <div className="space-y-4 px-5 py-5">
        {!cPhone && (
          <div className="flex items-center gap-2.5 rounded-xl border border-ember-500/35 bg-ember-500/10 px-3.5 py-3 text-[12.5px] font-bold text-ember-400">
            <I name="alert" size={15} /> {t("No phone number on this record — SMS can't be delivered.")}
          </div>
        )}
        {cOptOut && (
          <div className="flex items-center gap-2.5 rounded-xl border border-ember-500/35 bg-ember-500/10 px-3.5 py-3 text-[12.5px] font-bold text-ember-400">
            <I name="alert" size={15} /> {t("Client opted out of marketing SMS — transactional only (A2P 10DLC).")}
          </div>
        )}
        ...
        <Field label={t("Choose a template")}>
          <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
            {SMS_TEMPLATES.map((tp, i) => (
              <button key={tp.name} onClick={() => { setTpl(i); setTouched(false); }}
                ...
                <span className="mt-1 block truncate text-[10.5px] font-semibold text-ink-400">{tp.body}</span>
              </button>
            ))}
            <button onClick={() => { setTpl(0); setText(""); setTouched(true); }}
              ...
              <I name="plus" size={12} className="mr-1 inline" />{t("Blank message")}
            </button>
          </div>
        </Field>
        ...
        <Field label={t("Message preview — editable")}>
          <div className="relative">
            <textarea
              ...
              rows={5}
              placeholder={t("Write your message… merge fields fill automatically.")}
            ...
            <span className={`num absolute bottom-2.5 right-3 rounded-md px-1.5 py-0.5 text-[10px] font-bold ${len > 320 ? "bg-ember-500/15 text-ember-400" : "bg-ink-750 text-ink-400"}`}>
              {tf("{n} chars · {s} SMS", { n: len, s: segments })}
            </span>
          </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border-t border-ink-700 pt-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <Btn variant="gold" disabled={!body.trim() || !cPhone} onClick={send}>
              <I name="send" size={14} /> {t("Send via Twilio")}
            </Btn>
          </div>
        </div>
      </div>
