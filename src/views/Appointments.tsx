...
export function AppointmentDetail({ id, onBack }: { id: number; onBack: () => void }) {
  ...
  return (
    <div className="space-y-4 animate-rise">
      ...
      {play && <PlayerModal callId={play.id} durationHint={play.duration} result={play.result} title={appt.name} subtitle={`${play.ext} · ${fmtDT(play.startTime)}`} onClose={() => setPlay(null)} />}
    </div>
  );
}
